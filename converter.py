...
class Converter:
    ...
    def __init__(self, master):
        ...
        self.orderList.grid(row=1, column=0, rowspan=5, columnspan=4, padx=10, pady=10, sticky=(N, S, E, W))
        ...
        # Right Button Box
        self.getList = Button(master, text='Load Orders', command=lambda: self.generateList(self.oManager, self.orderList))
        self.getList.grid(row=1, column=4, padx=10, pady=10, sticky=(N, S, E, W))

        self.selectAll = Button(master, text='Select All', command=lambda: self.selectAllItems())
        self.selectAll.grid(row=2, column=4, padx=10, pady=10, sticky=(N, S, E, W))

        self.makeEDI = Button(master, text='Generate EDI', command=lambda: self.ediGenerator(self.oManager))
        self.makeEDI.grid(row=3, column=4, padx=10, pady=10, sticky=(N, S, E, W))

        self.upload = Button(master, text='Upload File', state=DISABLED, command=lambda: self.handleFTP())
        self.upload.grid(row=4, column=4, padx=10, pady=10, sticky=(N, S, E, W))

        self.submit = Button(master, text='Mark as Submitted', command=lambda: self.callSubmit())
        self.submit.grid(row=5, column=4, padx=10, pady=10, sticky=(N, S, E, W))
    ...
    def generateList(self, ordersManager, orderList):
        ...
        if (len(self.submittedOrders) > 0):
            for i in range(len(self.submittedOrders)):
                orderItem = self.submittedOrders[i].get('orderTag') + ", " + self.submittedOrders[i].get('PO') +", " +self.submittedOrders[i].get('email')
    ...
    def writeToFile(self, stringList):
        ...
        strConcat = " ".join(stringList).replace('&', '&amp;').replace('apos', '&apos;')
        f.write(strConcat)
        f.close()
    ...
    def handleFTP(self):
        ...
        try:
            ...
            # print(file_path)
            session.storbinary(f'STOR {self.fileName}.xml', file)     # send the file
